...
showMoreButton = driver.find_elements(By.XPATH, '//button[@class = "button button-show-more"]')
...
while True:
	try:
		...
		if showMoreButton:
			...
			print(f"Showing expansion page {pageNumber}")
			sleep(3)
			pageNumber += 1
		else:
			print("Show More button not found. Reached the last page.")
			break
...
for auction in auctionLinks:
	...
	key = str((VIN, date)) 
	auctionDataComplete[key] = auctionData
